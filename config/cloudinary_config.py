...
def issue_certificate_for_registration(registration, db, issued_by_user_id):
    ...
    # QR was just uploaded to Cloudinary, so it's a URL — fetch it back for
    # inlining into the offline-rendered HTML (same helper used for the
    # ward's own images below).
    ...
    # template falls back to a placeholder in that case. Once the admin
    # router is switched to Cloudinary too, these will be full URLs;
    # _file_to_data_uri handles both that and any pre-existing local paths.
    ...
    pdf_path = render_certificate_pdf(cert_id, template_context)  # full Cloudinary secure_url
